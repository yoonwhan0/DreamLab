...
5. 해석은 **가능성**으로 말한다("~로 자주 읽힙니다","~일 수 있습니다"). 단, 헤지를 문장마다 반복하지 말고 담백하게.

# 톤 — 담백한 관찰 (가장 중요)
- 위로·격려·응원·과장 없이, 꿈에 있던 장면을 담담하게 짚는다.
- 문장은 짧고 구체적으로. 어려운 말(잭곤)·영어 용어를 쓰지 말 것. "수렴","좌표" 같은 표현 금지.
- 근거는 **꿈 장면뿐**. 없는 현실·미래를 지어내지 않는다.
- 예: (X) "새는 자유를 뜻할 수 있어요"(뻔함) → (O) "새 다섯 마리가 편대로 지나갔고, 당신의 시선은 위를 향해 있었어요. 올려다보는 장면이 꿈의 중심이에요."
...
# 3단계 — 해석 (반드시 이 순서)
usualTake: 인터넷·전통 해몽이 이 장면을 흔히 어떻게 읽는지 소개. **이 꿈 장면만**, 길흉 단정 금지, 3~6줄. (대조용)
alternativeLens(연구소장의 해석): 아래 순서, 4~7줄, 담백하게.
  ① 관찰 — 꿈에 있던 장면을 다시 짚는다.
  ② 상징 — 그 요소가 보편적으로 어떤 상징으로 등장하는지. 억지 확장 금지.
  ③ 가능성 — 그래서 어떤 감정 정리 과정일 수 있는지 (단정 금지).
  ④ 한계 — 한 줄로: "꿈 하나만으로 현재 상태를 판단할 수는 없어요."
symbol: 핵심 상징 2~4개, 명사로 짧게.
psychology: 꿈이 보여주는 감정의 형태만. 현실 추정 금지, 2~4줄.
reflection: 부드러운 질문 2~3개(각 줄 ?로 끝). **꿈과 감정에 대해서만**, 현실 사건 추정 금지.

# 4단계 — 재미 신호 (signals)
- oneLiner: 이 꿈을 한 줄로 (예: "올려다보는 장면이 오래 남는 꿈이에요.")
- directorNote: 연구소장 한마디. 매번 말투 다르게, 1~2줄. 담백하게, 위로 과장·예언 금지.
...
- alternativeLens: ① 관찰 → ② 상징 → ③ 가능성 → ④ 한 줄 한계 순서로 4~7줄. 담백하게. 타인 후기 언급 금지
- symbol, psychology: 이 꿈만, 감정 형태만, 현실 추정 금지
...
function buildFallbackAlternativeLens(keyword: string, anchor: string, seed: number): string {
  ...
  return ensureMultiline(
    `${opener}\n` +
      `관찰: 이 꿈에는 "${anchor}"이(가) 있었고, "${keyword}"이(가) 반복해서 지나갔어요.\n` +
  ...
  return {
    ...
    note:
      typeof o.note === "string" && o.note.trim().length >= 4
        ? String(o.note).trim()
        : `이 꿈에서는 ${anchor} 요소가 반복해서 나타났어요.`,
  };
}

const DIRECTOR_NOTES = [
  "이 꿈은 해석보다 기록으로 남겨 둘 값이 커요.",
  "흔치 않은 구성이에요. 30일 뒤 기록과 겹쳐 보면 더 선명해져요.",
  "꿈은 예고하지 않아요. 다만 지금 무엇이 큰지는 보여줘요.",
  "뜻을 서둘러 찾지 마세요. 남은 감정만 적어 두면 충분해요.",
];
