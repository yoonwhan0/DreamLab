...
- usualTake: **인터넷 해몽처럼 자극적으로** (대박/손재/이별/시험 망침 등) — **이 꿈 장면만** 5~7줄
- alternativeLens: 연구소 **심리·상질** 관점 — **이 꿈만** 풀어 쓰기. 다른 사람 후기·통계·"비슷한 꿈 꾼 사람" **언급 금지**
- psychology, symbol, reflection — **당신 꿈** 감정·상질만. communityEstimate·labObservations에만 타인 데이터
...
## labObservations (타인·커뮤니티 패턴 — 여기만 FOMO)
...
function buildFallbackAlternativeLens(keyword: string, anchor: string, seed: number): string {
  ...
  return ensureMultiline(
    `${opener}\n` +
      ...
      `몸이 느낀 흥분·황홀함은 부끄럽지만, 꿈이 건드린 신호일 수 있어요.\n` +
